...
public class MappingAPI {
    ...
    public  static  List<String> showOnlyChanges;
        ...
        for (int i = 0; i < MainJavafiles.size(); i++) {
            ...
            for (int k = 0; k < CurrentJavaFile.size(); k++) {
                ...
                if (currentLine.contains("(") && currentLine.contains(")")) {
                   ...
                   // currentLine = "BluetoothAdapter.getAddress";
        ...
        try {
            ...
            if(METHOD_PARAMETER.equals("same")){
                ...
                if(Fullline.contains(checkline) && Fullline.contains(sameParameter)){
                    ...
                    if(ShowOnly){
                        showOnlyChanges.add("changes in "+details.getFileP().getName()+" in line number "+details.getLineNumber()+", Changes  :"+newLine);
                    }else {
                        lineEditor.replaceLine(details.getFileP(),details.getLineNumber(),newLine,true);
                        lineEditor.addNewLine(details.getFileP(), details.getLineNumber(), COMMENT+"  */",false);
                        lineEditor.addNewLine(details.getFileP(), details.getLineNumber(), "Refer this link to know more " + REFER_LINK,false);
                        lineEditor.addNewLine(details.getFileP(), details.getLineNumber(), "/*        Code Generated by AndroMigrate           ",false);
                        CurrentJavaFile = codeGenerator.readJavaFile(details.getFileP());
            ...
            if(Permission){
                ...
                if (!PERMISISON.equals("null")) {
                    ...
                    if (isPermissionAdded && removeLine !=0) {
                        ...
                        if(ShowOnly){
                            showOnlyChanges.add("changes in "+MainManifDetails.getFile().getName()+" in line number "+removeLine+", Line removed  :");
                        }else {
                            lineEditor.removeLine(MainManifDetails.getFile(), removeLine, true);
                            lineEditor.addNewLine(MainGradDetails.getFile(), removeLine, COMMENT, false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), removeLine, "<!-- Refer this link to know more " + REFER_LINK + "   -->", false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), removeLine, "<!--        Code Generated by AndroMigrate           -->", false);
                ...
                if(MANIF_APP_TYPE.equals("LIB")){
                    ...
                    if(!isLibAdded) {
                        if (ShowOnly) {
                            ...
                            lineEditor.removeLine(MainManifDetails.getFile(), removeLine, true);
                            lineEditor.addNewLine(MainManifDetails.getFile(), removeLine, "    " + COMMENT, false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), removeLine, "    <!-- Refer this link to know more " + REFER_LINK + "   -->", false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), removeLine, "    <!--        Code Generated by AndroMigrate           -->", false);
                            MainManifDetails = codeGenerator.readManifestFile(manifestDetails.getFile());
                        }
        ...
        try {
            ...
            if(isgradleChange){
                if (!GRADLE_CODE.equals("null")) {
                    ...
                    if(!isGradleLineAdded){
                        ...
                        if(ShowOnly){
                            showOnlyChanges.add("changes in "+MainManifDetails.getFile().getName()+" in line number "+defaultConfigLine+", Line added  :"+GRADLE_CODE);
                        }else {
                            lineEditor.addNewLine(MainGradDetails.getFile(), defaultConfigLine, "        " + GRADLE_CODE, true);
                            lineEditor.addNewLine(MainGradDetails.getFile(), defaultConfigLine, "         Refer this link to know more " + REFER_LINK + "   */", false);
                            lineEditor.addNewLine(MainGradDetails.getFile(), defaultConfigLine, COMMENT, false);
                            lineEditor.addNewLine(MainGradDetails.getFile(), defaultConfigLine, "/*        Code Generated by AndroMigrate          ", false);
            ...
            if(Permisison){
                if (!PERMISISON.equals("null")) {
                    ...
                    if (!isPermissionAdded) {
                        ...
                        if (ShowOnly) {
                            showOnlyChanges.add("changes in " + MainManifDetails.getFile().getName() + " in line number " + MainManifDetails.getUsesPermission().get(0).getLineNumber() + ", Line added  :" + PERMISISON);
                        } else {
                            lineEditor.addNewLine(MainManifDetails.getFile(), MainManifDetails.getUsesPermission().get(0).getLineNumber(), PERMISISON, true);
                            lineEditor.addNewLine(MainManifDetails.getFile(), MainManifDetails.getUsesPermission().get(0).getLineNumber(), "<!-- Refer this link to know more " + REFER_LINK + "   -->", false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), MainManifDetails.getUsesPermission().get(0).getLineNumber(), "<!--        Code Generated by AndroMigrate           -->", false);
                            MainManifDetails = codeGenerator.readManifestFile(manifestDetails.getFile());
                        }
                    }
                }
            ...
            if(provider){
                if(MANIF_APP_TYPE.equals("PROVIDER")){
                    ...
                    if(!isProvideAdded) {
                        if (ShowOnly) {
                            showOnlyChanges.add("changes in " + MainManifDetails.getFile().getName() + " in line number " + Applicationline + ", Line added  :" + MANIF_APP);
                        } else {

                            lineEditor.addNewLine(MainManifDetails.getFile(), Applicationline, "    " + MANIF_APP, true);
                            lineEditor.addNewLine(MainManifDetails.getFile(), Applicationline, "    <!-- Refer this link to know more " + REFER_LINK + "   -->", false);
                            lineEditor.addNewLine(MainManifDetails.getFile(), Applicationline, "    <!--        Code Generated by AndroMigrate           -->", false);
                            MainManifDetails = codeGenerator.readManifestFile(manifestDetails.getFile());
                        }
                    }
                }
